...
export const assessmentService = {
  ...
  getAvailable: async (cohortId) => {
    const response = await api.get(`/assessments/available?cohort_id=${cohortId}`);
    return response.data;
  },
...
export const windowService = {
  ...
  updateVisibility: async (id, visible) => {
    const response = await api.patch(`/assessment_windows/${id}/visibility`, { visible });
    return response.data;
  },
  ...
  updateLock: async (id, locked) => {
    const response = await api.patch(`/assessment_windows/${id}/lock`, { locked });
    return response.data;
  }
};
